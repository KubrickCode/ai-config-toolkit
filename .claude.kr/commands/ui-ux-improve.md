---
description: UI/UX 개선 조사 및 트렌드 분석으로 실행 가능한 권장사항 생성
---
...
```
이 프로젝트의 UI/UX를 다음 관점에서 분석:
1. 사용자 여정 및 흐름 최적화
2. 시각적 계층 구조 및 정보 구조
3. 접근성 준수 (WCAG 2.1 AA)
4. 컴포넌트 재사용성 및 일관성
5. 반응형 디자인 패턴
6. 사용자 인터랙션 피드백 패턴
...
- 스타일링: {스타일링 방식}
- 컴포넌트 라이브러리: {있는 경우}
- 집중 영역: {사용자 입력 또는 "전체 애플리케이션"}

우선순위와 함께 구체적이고 실행 가능한 권장사항 제공.
```
...
프로젝트 요구사항에 따라 다음 카테고리 평가:
1. 컴포넌트 라이브러리 (shadcn/ui, Radix, Headless UI 등)
2. 애니메이션 라이브러리 (Framer Motion, React Spring, GSAP)
3. 접근성 도구 (axe-core, react-aria 등)
4. 아이콘 라이브러리 (Lucide, Heroicons, Phosphor)
5. 폼 라이브러리 (React Hook Form, Formik 등)
...
집중 영역: {사용자 집중 영역 또는 "일반 UI/UX 개선"}
```
...
2. `"{프레임워크} UI design patterns 2025"` (예: "React UI design patterns 2025")
3. `"modern web design trends" accessibility`
4. `"{컴포넌트 유형} UX best practices"` (특정 집중 영역이 있는 경우)
5. `"best {라이브러리 유형} library 2025"` (예: "best React animation library 2025")
...
> **범위**: {full/targeted: 특정 영역}
> **모드**: {research/implement}
...
| 카테고리   | 라이브러리   | 점수   | 번들 크기 | 이유   |
| ---------- | ------------ | ------ | --------- | ------ |
| {카테고리} | {라이브러리} | {X}/10 | {크기}    | {이유} |
...
사용자가 `implement` 모드를 지정하거나 즉시 실행을 요청하면:

1. 진행 전 사용자에게 범위 확인
2. 최우선 순위 항목부터 시작
3. 명확한 커밋과 함께 점진적 변경
4. 각 변경이 기존 기능을 손상시키지 않는지 확인
5. 완료된 항목으로 보고서 업데이트
